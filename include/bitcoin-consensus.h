...
enum BIP9_INDEX
{
    ...
    BIP9_INDEX_DEPLOYMENT_SEGWIT, // Deployment of BIP141, BIP143, and BIP147.
    BIP9_INDEX_DEPLOYMENT_COUNT
};
...
typedef struct bitcoin_consensus
{
	...
	int bip65_height;
	unsigned char bip65_enabled_block_hash[32];
	int bip66_height;
	unsigned char bip66_enabled_block_hash[32];
	
	uint32_t rule_change_activation_threshold;
	uint32_t miner_confirmation_window;	
	bip9_deployment_t * bip9s;
	//~ bip9_deployment_t bip9s[BIP9_INDEX_DEPLOYMENT_COUNT];
...
extern const unsigned char mainnet_genesis_hash[32];
